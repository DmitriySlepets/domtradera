...
function getMainMenu(){
    ...
    $html = $html . '<div class="item"><a href="/brokers-search/index.php">Поиск Брокеры</a></div>';
...
function getPageBrokersSearch(){
    global $wpdb;
    if(isset($_GET['s'])){
        if($results = $wpdb->get_results( "SELECT * FROM kk_brokers WHERE title LIKE '%" . $_GET['s'] .  "%' || description LIKE '%" . $_GET['s'] .  "%'")){
    ...
    $form = '<form role="search" method="get" id="searchform"  action="/brokers-search/" >
    ...
    <main id="main" class="site-main brokers">
        <form role="search" method="get" id="searchform" action="/brokers-search/">
    ...
    <main id="main" class="site-main brokers">
        ...
        foreach ($results as $itemResult):
            ...
            try{
                ...
                //echo "width=" . $imgWidth . ";";
                //echo "height=" . $imgHeight . ";";
                if ($imgHeight1 == 0 or $imgWidth1 == $imgHeight1) {
                    $cover = "cover";
                } else {
                    $imgInteger1 = intdiv($imgWidth1, $imgHeight1);
                    //echo "integer=" . $imgInteger . ";";
                    $cover = ($imgInteger1 >= 1) ? "contain" : "cover";
                }
            }catch (Exception $ex) {
                //Выводим сообщение об исключении.
                //echo $ex->getMessage();
