...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	foreach($layout as $item_id=>$item_info){
		...
		if($item_key=='title'){
		    if($item_id==0){
                if($countMet==0){
                    $html_content.='<div class="element element_'.$item_id.' '.$item_key.'"  ><a href="'.get_permalink().'">';
                    $countMet = $countMet + 1;
                }else{
                    $html_content.='<div class="element element_'.$item_id.' '.$item_key.' second"  ><a href="'.get_permalink().'">';
                    $countMet=0;
                }
            }else{
                $html_content.='<div class="element element_'.$item_id.' '.$item_key.'"  ><a href="'.get_permalink().'">';
		...
		elseif($item_key=='read_more'){
			...
			if(!empty($item_info['read_more_text'])){
                ...
                $html_content .=  '<div target="'.$link_target.'" class="element element_'.$item_id.' '.$item_key.'">' .
                    '<div style="display:inline-block; margin-right: 20px;">' . newspaperly_posted_on_return() . '</div>' .
                    '<div style="display:inline-block;">' . the_tags_return() . '</div>' .
                    '</div>';
			}
